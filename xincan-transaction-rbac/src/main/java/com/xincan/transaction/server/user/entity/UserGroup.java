 ...
 * @Version: 1.0
 */
@ApiModel(value = "UserGroup", description = "用户组实体类")
@Data                       // 快捷生成Getter,Setter,equals,hashCode,toString函数
@Builder
@NoArgsConstructor          // 快捷生成无参构造函数
@AllArgsConstructor         // 快捷生成全参构造函数
@TableName(value = "user_group")   // 指定该类映射数据库表名
public class UserGroup {

    @ApiModelProperty(value="用户组ID", dataType = "String", example = "UUID")
    @NotBlank(message = "用户组ID不能为空")
    @Length(message = "用户组ID长度应该在{min}~{max}之间", min = 1, max = 64)
    @TableId(type = IdType.UUID)
    @TableField("id")
    private String id;

    @ApiModelProperty(value="用户组名称", dataType = "String", required = true, example = "超级管理员")
    @NotBlank(message = "用户登录名称不能为空")
    @Length(message = "用户登录名称长度应该在{min}~{max}之间", min = 1, max = 30)
