...
import com.xincan.transaction.log.enumer.ESystemBusinessLogType;
import com.xincan.transaction.log.enumer.ESystemLogType;
...
public class SystemLog {
    ...
    @Length(message = "当前系统操作用户所属公司名称长度应该在{min}~{max}之间", min = 1, max = 100)
    @TableField(value = "company")
    private String company;

    @ApiModelProperty(value="当前系统操作用户所在部门名称", dataType = "String", example = "视觉交互部")
    @NotBlank(message = "当前系统操作用户所在部门名称不能为空")
    @Length(message = "当前系统操作用户所在部门名称长度应该在{min}~{max}之间", min = 1, max = 100)
    ...
    @Length(message = "当前系统操作主机IP长度应该在{min}~{max}之间", min = 1, max = 100)
    @TableField(value = "ip")
    private String ip;
    ...
    @ApiModelProperty(value="当前系统微服务名称", dataType = "String", example = "xincan-transaction-user")
    @NotBlank(message = "当前系统微服务名称不能为空")
    @Length(message = "当前系统微服务名称长度应该在{min}~{max}之间", min = 1, max = 50)
    @TableField(value = "micro_service")
    private String microService;

    @ApiModelProperty(value="当前系统请求微服务方法函数路径", dataType = "String", example = "http://192.168.1.1:8080/user/find")
    ...
    @ApiModelProperty(value="当前系统类请求微服务方法函数参数", dataType = "String", example = "[{\"name\":\"lisi\"}]")
    @TableField(value = "class_param")
    private String classParam;

    @ApiModelProperty(value="当前系统操作类型", dataType = "ESystemLogType", notes = "com.xincan.transaction.log.enumer.ESystemLogType", required = true, example = "增加、修改、删除、导入、导出")
    @NotNull(message = "当前系统操作类型不能为空")
    @TableField(value = "type")
    private ESystemLogType type;

    @ApiModelProperty(value="当前系统业务日志类型", dataType = "ESystemBusinessLogType", notes = "com.xincan.transaction.log.enumer.ESystemBusinessLogType", required = true, example = "登录日志，操作日志，流程日志，脚本日志")
